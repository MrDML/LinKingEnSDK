...
@class TWSwizzler;
@interface TWDependency : NSObject
+ (instancetype)shared;
...
@property (nonatomic,copy)void(^bindingAccountCompleteCallBack)(TWSwizzler *user,NSError *error);




- (void)login:(UIViewController*)rootViewController onFinished:(void(^)(TWSwizzler *user,NSError *error))complete;

- (void)logout;



- (void)loginApiWithRootViewController:(UIViewController *_Nullable)viewController complete:(void(^)(TWSwizzler *user,NSError *error))complete;
